...
import {
  Zap, RefreshCw, Activity, AlertTriangle, TrendingUp, Ship,
  Globe, ArrowLeft, ExternalLink, Newspaper, Radio, Search,
...
interface LiveIntelligence {
  ...
  latestAlerts: Array<{
    id: string; hotspot: string; severity: string
    message: string; source: string; timestamp: string
  }>
  marketNews: Array<{
    id: string; category: string; title: string
    summary: string; source: string; timestamp: string
  }>
  timestamp: string
}
...
// Daily transits sourced from EIA, UNCTAD, SCA & MPA Q1 2026 reports
...
const maritimeData: Record<string, MaritimeHotspot> = {
  hormuz: {
    ...
    zoom: 8,
    // ~21M bbl/day oil = 17–20 VLCCs + tankers + dry cargo ≈ 56 ships/day
  ...
  bab: {
    ...
    zoom: 8,
    // Reduced from ~65 to ~42/day due to Houthi attacks rerouting vessels via Cape
  ...
  malacca: {
    ...
    zoom: 9,
    // ~90,000 ships/year ÷ 365 ≈ 248/day (world's busiest strait)
  ...
  suez: {
    ...
    zoom: 9,
    // Reduced from ~80 to ~52/day due to Red Sea diversions avoiding Bab el-Mandeb
    ...
    alerts: [
      ...
      { severity: "warning", message: "SCA Security: Enhanced screening for vessels from Bab el-Mandeb. Allow 4hr additional clearance time." },
...
const vesselConfig = {
  tanker:    { color: "#FF6B6B", label: "T" },
  cargo:     { color: "#4ECDC4", label: "C" },
  container: { color: "#45B7D1", label: "B" },
  lng:       { color: "#96CEB4", label: "L" },
}
...
export default function MapDashboardPage() {
  ...
  useEffect(() => {
    ...
    const apiInterval   = setInterval(() => fetchLiveIntelligence(), 60000)
    const newsInterval  = setInterval(() => fetchMaritimeNews(), 30000)
    return () => {
                  ...
                  <div className="rounded-xl bg-accent/10 border border-accent/20 p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-2xl font-bold text-accent">
                        {vessels.length > 0
                          ? (vessels.reduce((s, v) => s + v.speed, 0) / vessels.length).toFixed(1)
                          : "0.0"}
                      </span>
                    </div>
                  ...
                  <div key={news.id} className="rounded-xl p-4 border bg-card/50 border-border">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {news.summary && (
                          <div className="mt-0.5 text-xs text-muted-foreground">{news.summary}</div>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="rounded-xl p-3 border border-dashed border-border bg-background/50">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-muted-foreground font-mono">
                      {liveIntel?.timestamp
                        ? new Date(liveIntel.timestamp).toLocaleTimeString()
                        : "--:--"}
                    </span>
                  </div>
                </div>
            ...
            <div className="mt-6 glass rounded-2xl border border-border p-6 animate-in slide-in-from-top-4 duration-300">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Live updates from industry sources</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
              ...
              {marketData && (
                <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="rounded-xl bg-card/50 border border-border p-3">
                    <div className="text-xs text-muted-foreground mb-1">Active Vessels</div>
