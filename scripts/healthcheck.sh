...
if [ "$HTTP_STATUS" -ge 200 ] && [ "$HTTP_STATUS" -lt 500 ]; then
    ...
    echo "$BODY"
    SUMMARY=$(printf "%s" "$BODY" | node -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{try{const j=JSON.parse(s);const c=j.components||{};const q=c.sourceQuality||{};process.stdout.write([j.status||'unknown',c.cache?.status||'unknown',c.coverage?.status||'unknown',q.status||'missing',q.auditStatus||'missing',(q.recommendations||[])[0]||''].join('\t'))}catch{process.stdout.write('invalid-json\tunknown\tunknown\tmissing\tmissing\t')}})")
    IFS=$'\t' read -r HEALTH_STATUS CACHE_STATUS COVERAGE_STATUS SOURCE_QUALITY_STATUS SOURCE_AUDIT_STATUS SOURCE_QUALITY_HINT <<< "$SUMMARY"
    echo "Components: cache=$CACHE_STATUS coverage=$COVERAGE_STATUS sourceQuality=$SOURCE_QUALITY_STATUS audit=$SOURCE_AUDIT_STATUS"
